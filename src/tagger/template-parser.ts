...
export function getChildShape(element: VisitedElement): string[] {
  ...
  for (const child of element.children ?? []) {
    ...
    const key = extractShallowChildKey(child);
    out.push(key ? `${tag}:${key}` : tag);
  }
  return out;
}
...
function extractShallowChildKey(element: VisitedElement): string | null {
  const candidates: ReadonlyArray<string | undefined | null> = [
    ...
    findAttribute(element, 'name')?.value,
    findAttribute(element, 'aria-label')?.value,
    findAttribute(element, 'label')?.value,
    findAttribute(element, 'placeholder')?.value,
    findAttribute(element, 'routerlink')?.value,
    findAttribute(element, 'title')?.value,
    findAttribute(element, 'href')?.value,
    findAttribute(element, 'alt')?.value,
    getStaticTextContent(element),
    findAttribute(element, 'value')?.value,
    findAttribute(element, 'type')?.value,
    findAttribute(element, 'role')?.value,
    findAttribute(element, 'id')?.value
  ];
